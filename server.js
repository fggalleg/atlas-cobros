...
function sendEmail(to, subject, html) {
  return new Promise((resolve, reject) => {
    const body = JSON.stringify({ from: 'Atlas Cobros <noreply@payfastsolutions.com>', to, subject, html });
    const options = {
...
app.post('/api/send-email', async (req, res) => {
  ...
  try {
    ...
    if (result.id) { console.log('CORREO ENVIADO:', result.id); res.json({ success: true }); }
    else { console.error('ERROR CORREO:', JSON.stringify(result)); res.status(500).json({ error: result }); }
  } catch (e) { console.error('ERROR CORREO:', e); res.status(500).json({ error: e.toString() }); }
});
...
app.post('/api/notify-code', async (req, res) => {
  ...
  } catch (e) { console.error('ERROR NOTIFICACION:', e); res.status(500).json({ error:
